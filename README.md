...
Backend runs at `http://localhost:8080` (open `/` or `/health` to verify it is reachable).
...
- `POST /api/drivers/:driverId/status` (requires route start/end coordinates + rough driver location when active)
