...
class RickshawPulseApp extends StatelessWidget {
  ...
  void initState() {
    super.initState();
    routeStartLatController.text = '23.7806';
    routeStartLonController.text = '90.4070';
    routeEndLatController.text = '23.7500';
    routeEndLonController.text = '90.3900';
    driverLatController.text = '23.7806';
    driverLonController.text = '90.4070';
  }
...
class _DriverScreenState extends State<DriverScreen> {
  ...
  final routeStartLatController = TextEditingController();
  final routeStartLonController = TextEditingController();
  final routeEndLatController = TextEditingController();
  final routeEndLonController = TextEditingController();
  final driverLatController = TextEditingController();
  final driverLonController = TextEditingController();
  String? driverId;
  bool active = false;
  ...
  Future<void> toggleStatus(bool value) async {
    ...
    await http.post(
      ...
      body: jsonEncode({
        'active': active,
        'lat': double.tryParse(driverLatController.text),
        'lon': double.tryParse(driverLonController.text),
        'routeStartLat': double.tryParse(routeStartLatController.text),
        'routeStartLon': double.tryParse(routeStartLonController.text),
        'routeEndLat': double.tryParse(routeEndLatController.text),
        'routeEndLon': double.tryParse(routeEndLonController.text),
      }),
    );
  }
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text('Route coordinates (same route travelers query):'),
        TextField(controller: routeStartLatController, decoration: const InputDecoration(labelText: 'Route start lat')),
        TextField(controller: routeStartLonController, decoration: const InputDecoration(labelText: 'Route start lon')),
        TextField(controller: routeEndLatController, decoration: const InputDecoration(labelText: 'Route end lat')),
        TextField(controller: routeEndLonController, decoration: const InputDecoration(labelText: 'Route end lon')),
        const SizedBox(height: 8),
        const Text('Your rough current location:'),
        TextField(controller: driverLatController, decoration: const InputDecoration(labelText: 'Driver lat')),
        TextField(controller: driverLonController, decoration: const InputDecoration(labelText: 'Driver lon')),
