...
app.post('/api/drivers/:driverId/status', (req, res) => {
  ...
  const { active, lat, lon, routeStartLat, routeStartLon, routeEndLat, routeEndLon } = req.body;
  const db = readDb();
  ...
  if (driver.active) {
    if ([routeStartLat, routeStartLon, routeEndLat, routeEndLon, lat, lon].some((v) => Number.isNaN(Number(v)))) {
      return res.status(400).json({ error: 'route and driver coordinates are required when active' });
    }

    db.activity.push({
      id: id('act'),
      driverId,
      route: routeKey(routeStartLat, routeStartLon, routeEndLat, routeEndLon),
...
app.get('/api/availability', (req, res) => {
  ...
  const sampleCount = Math.max(10, db.activity.filter((a) => a.route === route).length);
  const label = availabilityLabel(byHour.length, sampleCount);
  res.json({
    route,
